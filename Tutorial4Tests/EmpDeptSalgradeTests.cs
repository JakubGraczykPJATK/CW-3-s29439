...
public class EmpDeptSalgradeTests
{
    ...
    public void ShouldReturnEmployeesFromChicago()
    {
        ...
        List<Emp> result = emps.Where(s => depts.Where(d => d.Loc == "CHICAGO").Any(d => d.DeptNo == s.DeptNo))
            .ToList();
    ...
    public void ShouldJoinEmployeesWithDepartments()
    {
        ...
        var result = emps.Join(depts, emp => emp.DeptNo, dep => dep.DeptNo, (emp, dep) => new
        {
            dep.DName,
            emp.EName
        });
    ...
    public void ShouldMatchEmployeeToSalaryGrade()
    {
        ...
        var result = emps.SelectMany(e =>
                grades.Where(s => e.Sal >= s.Losal && e.Sal <= s.Hisal),
            (e, s) => new
            {
                EName = e.EName,
                Grade = s.Grade
